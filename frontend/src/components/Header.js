        ...
        const fetchData = () => {
            ...
            if (keyword.length > 2) {
                timer = setTimeout(async () => {
                    try {
                        const response = await sendRequest(`${process.env.REACT_APP_API}/api/v1/show/search?keyword=${keyword}`)
    ...
    return (
        <header>
            <div className="header-buttons">
                ...
                {!user ? <Link to="/login">Login</Link> : <a href={`${process.env.REACT_APP_API}/logout`}>Logout</a>}
            </div>
