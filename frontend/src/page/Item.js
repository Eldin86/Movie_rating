...
const Item = () => {
    ...
    const changeRating = async (rating) => {
        setRating(rating)
        try {
            const res = await sendRequest(`${process.env.REACT_APP_API}/api/v1/show/${ID}/rating`, 'POST', { rating })
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await sendRequest(`${process.env.REACT_APP_API}/api/v1/show/${ID}`)
